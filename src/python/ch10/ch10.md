...
## 10.9 seq2seq 与 encoder-decoder

当输入和输出都是不定长序列时，可以使用

-   seq2seq：序列到序列模型
-   encoder-decoder：编码器-解码器模型，编码器用来分析输入序列，解码器用来生成输出序列

### 10.9.1 编码器

把一个不定长的输入序列变换成一个定长的背景变量，并在该背景变量中编码输入序列的信息。

编码器的构造

-   单向循环神经网络，每个时间步的隐藏状态只取决于该时间步及之前的输入子序列
-   双向循环神经网络，每个时间步的隐藏状态同时取决于该时间步及之前和之后的输入子序列，因此编码了整个序列的信息

假设输入序列为 $x_1,\cdots,x_T$

1.  在时间步   $t$，循环神经网络将输入 $x_t$ 的特征向量 $\mathbf{x}_t$ 和上个时间步的隐藏状态 $\mathbf{h}_{t-1}$ 变换为当前时间步的 $\mathbf{h}_t$，即：$\mathbf{h}_t=f(\mathbf{x}_t,\mathbf{h}_{t-1})$
2.  编码器通过自定义函数 $q$ 将各个时间步的隐藏状态变换为背景变量：$\mathbf{c}=q(\mathbf{h}_1,\cdots,\mathbf{h}_T)$
3.  背景变量是输入序列最终时间步的隐藏状态 $\mathbf{h}_T$



### 10.9.2 解码器

解码器基于之前的输出序列和背景变量输出接下来的样本

编码器输出的背景向量 $\mathbf{c}$ 编码了整个输入序列 $x_1,\cdots,x_T$ 的信息。给定训练样本中的输出序列 $y_1,y_2,\cdots,y_{T'}$，对于每个时间步 $t'$（输出序列的时间步与输入序列或者编码器的时间步是不同的），解码器输出 $y_{t'}$ 的条件概率将基于之前的输出序列 $y_1,\cdots,y_{t'-1}$ 和背景变量 $\mathbf{c}$ ，即 $P(y_{t'}|y_1,\cdots,y_{t'-1},\mathbf{c})$。使用另一个循环神经网络在输出序列的时间步 $t'$，解码器将上一时间步的输出 $y_{t'-1}$ 和 背景变量 $\mathbf{c}$ 作为输入，并将它们与上一时间步的隐藏状态 $\mathbf{s}_{t'-1}$ 利用变换函数 $g$ 变换为当前时间步的隐藏状态 $\mathbf{s}_{t'}$，即：$\mathbf{s}_{t'}=g(y_{t'-1},\mathbf{c},\mathbf{s}_{t'-1})$。然后，使用自定义输出层和 Softmax 运算来计算概率 $P(y_{t'}|y_1,\cdots,y_{t'-1},\mathbf{c})$。

### 10.9.3 训练模型

根据最大似然估计，最大化输出序列基于输入序列的条件概率
$$
\begin{aligned}
P(y_1,\cdots,y_{T‘}|x_1,\cdots,x_T)
	&=\prod_{t'=1}^{T'}P(y_{t'}|y_1,\cdots,y_{t'-1},x_1,\cdots,x_T)\\
	&=\prod_{t'=1}^{T'}P(y_{t'}|y_1,\cdots,y_{t'-1},\cdots,y_{t'-1},\mathbf{c})
\end{aligned}
$$

得到这个输出序列的损失
$$
-\log P(y_1,\cdots,y_{T'}|x_1,\cdots,x_T)=-\sum_{t'-1}^{T'}\log P(y_{t'}|y_1,\cdots,y_{t'-1},\mathbf{c})
$$
在训练中如果将标签序列（训练集的真实输出序列）在上一个时间步的标签作为解码器在当前时间步的输入，这属于强制教学或者强化学习（teacher forcing）。

### 小结

1.  编码器-解码器可以输入并且输出不定长的序列
2.  编码器-解码器使用了两个循环神经网络
3.  编码器-解码器的训练中可以使用强制教学

## 10.10 束搜索

假设解码器的输出是一段文本序列。设输出文本词典 $\mathcal{Y}$ 的大小为 $|\mathcal{Y}|$，词典中包含特殊符号「`<eos>`」，输出序列的最大长度为 $T'$。所有可能的输出序列共有 $\mathcal{O}(|\mathcal{Y}|^{T'})$种，这些输出序列中所有特殊符号「`<eos>`」后面的子序列都将被舍弃。

### 10.10.1 贪婪搜索

贪婪搜索（greedy search）：对于输出序列任一时间步 $t'$，从 $|\mathcal{Y}|$ 个词中搜索出条件概率最大的词 $y_{t'}=\arg\max P(y|y_1,\cdots,y_{t'-1},\mathbf{c})$ 作为输出，直到遇见「`<cos>`」或者输出序列长度已经达到最大长度 $T'$，便完成输出。缺点：输出不能保证得到最优序列

### 10.10.2 穷举搜索

穷举搜索（exhaustive search）：穷举所有可能的输出序列，输出条件概率最大的序列。缺点：计算代价过大，理论可行。

### 10.10.3 束搜索

束搜索（beam search）：对贪婪搜索的改进，使用一个束宽（beam size）超参数 $k$。

-   在时间步 1 时，选取当前时间步条件概率最大的 $k$ 个词，分别组成 $k$ 个候选输出序列的首词
-   在之后的每个时间步，基于上个时间步的 $k$ 个候选输出序列，从 $k|\mathcal{Y}|$ 个可能的输出序列中选取条件概率最大的 $k$ 个，作为这个时间的候选输出序列。
-   最终，从各个时间步的候选输出序列中筛选出包含特殊符号「`<eos>`」的序列，并将所有特殊符号后面的子序列舍弃，得到最终候选输出序列的集合。
-   基于最终候选输出序列的集合，选取得分最高的序列作为输出序列
    -   $L$为最终候选序列长度，$\alpha$ 一般选 0.75
    -   评分函数：$\frac1{L^\alpha}\log P(y_1,\cdots,y_L)=\frac1{L^\alpha}\sum_{t'=1}^L\log P(y_{t'}|y_1,\cdots,y_{t'-1},\mathbf{c})$
-   束搜索的计算开销为：$\mathcal{O}(k|\mathcal{Y}T')$
    -   贪婪搜索为束宽为 1 的束搜索
    -   穷举搜索为束宽为 $|\mathcal{Y}|$ 的束搜索

### 小结

1.  预测不定长序列的方法包括贪婪搜索、穷举搜索和束搜索
2.  束搜索通过灵活的束宽来权衡计算开销和搜索质量

## 10.11 注意力机制

传统的编码器为循环神经网络时，背景变量为最终时间步的隐藏状态。解码器在各个时间步依赖相同的背景变量来获取输入序列信息。

在注意力机制中，通过对编码器所有时间步的隐藏状态做加权平均来得到背景变量。解码器在每一时间步调整这些权重，即注意力权重，从而能够在不同时间步分别关注输入序列中的不同部分，并且编码进相应时间步的背景变量。

传统的编码器区分了输入序列或者编码器的索引 $t$ 与输出序列或者解码器的索引 $t'$。解码器在时间步 $t’$ 的隐藏状态 $\mathbf{s}_{t'}=g(\mathbf{y}_{t'-1},\mathbf{c},\mathbf{s}_{t'-1})$，其中 $\mathbf{y}_{t'-1}$ 是上一时间步 $(t'-1)$ 的输出 $y_{t'-1}$ 的表征 ，并且任一时间步 $t'$ 使用相同的背景变量 $\mathbf{c}$。

注意力机制中，解码器的每一时间步将使用可变的背景变量。记 $\mathbf{c}_{t'}$ 是解码哭喊 在时间步 $t'$ 的背景变量，那么解码器在该时间步的隐藏状态可以改写为 $\mathbf{s}_{t'}=g(\mathbf{y}_{t'-1},\mathbf{c}_{t'},\mathbf{s}_{t'-1})$。

### 10.11.1 计算背景变量

图10-12 编码器-解码器上的注意力机制

1.  令编码器在时间步 $t$ 的隐藏状态为 $\mathbf{h}_t$，并且总的时间步数为 $T$，则解码器在时间步 $t'$ 的背景变量为所有编码器隐藏状态的加权平均：$\mathbf{c}_{t'}=\sum_{t=1}^T\alpha_{t't}\mathbf{h}_t$。
2.  给定 $t'$时，权重 $\alpha_{t't},t=1,\cdots,T$ 是一个概率分布。
3.  为了得到概率分布，使用 Softmax 运算：$\alpha_{t't}=\frac{\exp(e_{t't})}{\sum_{k=1}^T\exp(e_{t'k})},t=1,\cdots,T$ 
4.  因为 $e_{t't}$ 取决于编码器的时间步 $t'$ 和编码器的时间步 $t$，则基于函数 $a(\cdot)$ 计算：$e_{t't}=a(\mathbf{s}_{t'-1},\mathbf{h}_t)$，其中 $\mathbf{s}_{t’-1}$ 为解码器在时间步 $(t'-1)$ 的隐藏状态，$\mathbf{h}_t$ 为编码器在时间步 $t$ 的隐藏状态
5.  函数 $a(\cdot)$ 的选择
    1.  计算内积：$a(\mathbf{s,h})=\mathbf{s}^T\mathbf{h}$
    2.  最早提出注意力机制的论文将输入连结后通过单隐藏层的多层感知机变换：$a(\mathbf{s,h})=\mathbf{v}^T\tanh(\mathbf{W}_s\mathbf{s}+\mathbf{W}_h\mathbf{h})$
        1.  其中的 $\mathbf{v}, \mathbf{W}_s,\mathbf{W}_h$ 都是可以学习的模型参数
    3.  矢量化计算：输入包括查询项及完全对应的键项和值项
        1.  值项是需要加权平均的一组项。在加权平均中，值项的权重来自查询项以及与该值项对应的键项
        2.  查询项为解码器的隐藏状态
        3.  键项和值项为编码器的隐藏状态
        4.  设编码器和解码器的隐藏单元个数为 $h$，并且函数为 $a(\mathbf{s,h})=\mathbf{s}^T\mathbf{h}$
            1.  如果希望根据解码器单个隐藏状态 $\mathbf{s}_{t'-1}\in\mathcal{R}^h$ 和编码器所有隐藏状态 $\mathbf{h}_t\in\mathcal{R}^h,t=1,\cdots,T$ 来计算背景向量 $\mathbf{c}_{t'}\in\mathcal{R}^h$
            2.  将查询项矩阵 $\mathbf{Q}\in\mathcal{R}^{1\times h}$ 设为 $\mathbf{s}_{t'-1}^T$
            3.  令 键项矩阵 $\mathbf{K}\in\mathcal{R}^{T\times h}$ 和 值项矩阵 $\mathbf{V}\in\mathcal{R}^{T\times h}$ 相同，并且第 $t$ 行均为 $\mathbf{h}_t^T$
            4.  通过失量化计算：$\text{softmax}(\mathbf{QK}^T)\mathbf{V}$ 即可得到转置后的背景向量 $\mathbf{c}_{t'}^T$
            5.  当查询项矩阵 $\mathbf{Q}$ 的行数为 $n$ 时，矢量化计算的输出矩阵与查询项矩阵在相同行上完全对应

### 10.11.2 更新隐藏状态

以门控循环单元为例

-   解码器在时间步 $t’$ 的隐藏状态为：$\mathbf{s}_{t'}=\mathbf{z}_{t'}\odot\mathbf{s}_{t'-1}+(1-\mathbf{z}_{t’})\odot\tilde{\mathbf{s}}_{t'}$
-   重置门：$\mathbf{r}_{t'}=\sigma(\mathbf{W}_{yr}\mathbf{y}_{t'-1}+\mathbf{W}_{sr}\mathbf{s}_{t'-1}+\mathbf{W}_{cr}\mathbf{c}_{t'}+\mathbf{b}_r)$
-   更新门：$\mathbf{z}_{t'}=\sigma(\mathbf{W}_{yz}\mathbf{y}_{t'-1}+\mathbf{W}_{sz}\mathbf{s}_{t'-1}+\mathbf{W}_{cz}\mathbf{c}_{t'}+\mathbf{b}_r$
-   候选隐藏状态：$\tilde{\mathbf{s}}_{t'}=\tanh(\mathbf{W}_{ys}\mathbf{y}_{t'-1}+\mathbf{W}_{ss}(\mathbf{s}_{t'-1}\odot\mathbf{r}_{t'})+\mathbf{W}_{cs}\mathbf{c}_{t'}+\mathbf{b}_s)$
-   包含下标的 $\mathbf{W,b}$ 分别为门控循环单元的权重参数和偏差参数

### 小结

1.  可以在解码器的每个时间步使用不同的背景变量，并且对输入序列中不同时间步编码的信息分配不同的注意力
2.  注意力机制的输入包括查询项以及完全对应的键项和值项
3.  注意力机制可以采用更为高效的失量化计算

## 10.12 机器翻译

注意力机制的输入包括：查询项、键项、值项。

-   查询项：解码器在上一时间步的隐藏状态，形状为（批量大小，隐藏单元个数）
-   键项和值项：编码器在所有时间步的隐藏状态，形状为（时间步数，批量大小，隐藏单元个数）
-   注意力机制返回当前时间步的背景向量，形状为（批量大小，隐藏单元个数）

### 10.12.5 评价翻译结果

BLEU（Bilingual Evaluation Understudy）的定义：

-   设词数为 $n$ 的子序列的精度为 $p_n$，等于预测序列与标签序列匹配词数为 $n$ 的子序列的数量与预测序列中词数为 $n$ 的子序列的数量之比。
-   $k$ 是希望匹配的子序列的最大词数，
-   $len_{label}$ 是标签序列的词数，
-   $len_{pred}$ 是预测序列的词数。
